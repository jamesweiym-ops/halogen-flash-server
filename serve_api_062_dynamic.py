...
def request_budget(req, value):
    """Omitted/null budgets use remaining context; explicit limits still win."""
...
def build_app(tok, engine, ctx, max_cap=4096, queue_timeout=600):
    ...
    async def health():
        ...
        body = {"status": "ok" if live else "engine_unresponsive",
                ...
                "output_budget_policy": "omitted/null: min(cap, context - rendered_prompt_tokens - 1024); explicit: respected",
    ...
    async def serve(ids, max_tokens, stops, stream, chat, prefix,
        ...
        print(f"serve_api: output_budget prompt={len(ids)} context={limit} "
              f"effective={max_tokens} automatic={automatic}", flush=True)

...
export function openaiResponsesToOpenAIResponse(chunk, state) {
  ...
  // Halogen emits the full reasoning stream as response.reasoning_text.delta
  // when the upstream request does not ask for a Responses summary. Chat
  // clients still expect the OpenAI-compatible reasoning_content field.
  // When a summary part exists, Halogen emits both raw and summary deltas;
  // prefer the summary path in that case so the client does not see duplicates.
  if (eventType === "response.reasoning_summary_part.added") {
    state.reasoningSummaryPartSeen = true;
    return null;
  }

  if (eventType === "response.reasoning_text.delta") {
    const delta = data.delta || "";
    if (!delta || state.reasoningSummaryPartSeen) return null;
    state.reasoningRawTextSeen = true;
    return buildChunk(
      { id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK },
  ...
  if (eventType === "response.reasoning_summary_text.delta") {
    const delta = data.delta || "";
    if (!delta || state.reasoningRawTextSeen) return null;
    return buildChunk(
      { id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK },
      reasoningDelta(delta)
    );

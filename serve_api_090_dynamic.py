...
def request_budget(req, value):
    """Respect explicit budgets; omitted budgets use remaining context."""
    names = ("max_tokens", "max_completion_tokens", "max_output_tokens")
    return value if any(n in req.model_fields_set and
                        getattr(req, n, None) is not None for n in names) else None


def remaining_budget(prompt_tokens, context, cap, margin=1024):
    room = min(cap, context - prompt_tokens - margin)
    if room < 1:
        raise HTTPException(400, f"prompt {prompt_tokens} leaves no output room "
                            f"in context {context} with safety margin {margin}")
    return room
...
def build_app(tok, engine, ctx, max_cap=4096, queue_timeout=600):
    ...
    async def health():
        ...
        body = {"status": "ok" if live else "engine_unresponsive",
                ...
                "local_patch": "remaining-context-budget-v1",
                "output_budget_policy": "omitted: min(cap, context - rendered_prompt_tokens - 1024); explicit: respected",
                "output_budget_safety_margin": 1024,
    ...
    async def serve(ids, max_tokens, stops, stream, chat, prefix,
        ...
        automatic = max_tokens is None
        want = (remaining_budget(len(ids), engine.info.get("ctx") or ctx, max_cap)
                if automatic else int(max_tokens))
        print(f"serve_api: output_budget prompt={len(ids)} effective={want} "
              f"automatic={automatic}", flush=True)
        if want > max_cap:
            raise HTTPException(
    ...
    async def completions(req: CompletionReq):
        ...
        check_sampling(req)
        return await serve(ids, request_budget(req, req.max_tokens), stop_list(req.stop),
    ...
    async def responses(req: ResponsesReq):
        ...
        if pre:
            ids += tok(pre, add_special_tokens=False)["input_ids"]
        return await serve(ids, request_budget(req, shim.max_tokens), stop_list(None),

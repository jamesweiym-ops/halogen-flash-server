...
def _label(ts, unit):
    lt = time.gmtime(float(ts) + TZ_OFFSET_H * 3600.0)
    if unit == "hour":
        return time.strftime("%m-%d %H:00", lt)
    if unit == "week":
...
def _bucket_labels(unit, count):
    now = time.time() + TZ_OFFSET_H * 3600.0
    labels = []
    if unit == "hour":
        base = int(now // 3600)
        labels = [time.strftime("%m-%d %H:00", time.gmtime((base - i) * 3600))
                 for i in range(count - 1, -1, -1)]
    elif unit == "week":
        day = int(now // 86400)
        monday = day - ((day + 3) % 7)
        labels = [time.strftime("%G-W%V", time.gmtime((monday - i * 7) * 86400))
                 for i in range(count - 1, -1, -1)]
    elif unit == "month":
        lt = time.gmtime(now)
...
class RequestLedger:
    ...
    def usage(self, unit, count):
        ...
        for r in self._load():
            ...
            b["requests"] += 1
            b["prompt"] += int(r.get("prompt_tokens", 0))
            b["cached"] += int(r.get("cached_tokens", 0))
            b["output"] += int(r.get("output_tokens", 0))
            b["draft"] += int(r.get("draft_n", 0))
            b["draft_accepted"] += int(r.get("draft_n_accepted", 0))
        return [buckets.get(l, dict(empty, label=l)) for l in _bucket_labels(unit, count)]
    ...
    <div class="chartcard">
      <div class="chart-head">
        ...
        <div class="units" id="units">
          ...
          <button data-u="week" onclick="setUnit('week')">按周</button>
          <button data-u="month" onclick="setUnit('month')">按月</button>
        </div>
      </div>
      <canvas id="cv"></canvas>
      <div class="legend">
...
function setUnit(u){unit=u;document.querySelectorAll('#units button').forEach(b=>b.classList.toggle('active',b.dataset.u===u));const nm={hour:'按小时',day:'按日',week:'按周',month:'按月'}[u];document.getElementById('chartTitle').textContent=nm+'统计';loadUsage();}
async function jget(u){const r=await fetch(u);return r.json();}
async function loadTotals(){
  try{const d=(await jget('/api/report/totals')).data||{};
    const hr=d.hit_rate!=null?(d.hit_rate*100).toFixed(1)+'%':'—';
    document.getElementById('cards').innerHTML=
      card('总请求',fmt(d.requests))+
      card('总输入 Token',fmt(d.prompt))+
      card('总命中缓存',fmt(d.cached),'小')+
      card('缓存命中率',hr)+
      card('总输出 Token',fmt(d.output))+
      card('草稿接受',fmt(d.draft_accepted)+' / '+fmt(d.draft));
  }catch(e){document.getElementById('cards').innerHTML='<div class="err">汇总加载失败: '+esc(e.message)+'</div>';}
}
function card(k,v,s){return '<div class="card"><div class="k">'+k+'</div><div class="v">'+v+(s?'<small>'+s+'</small>':'')+'</div></div>';}
async function loadUsage(){
  const cnt={hour:48,day:30,week:12,month:12}[unit];
  let d=[];try{d=(await jget('/api/report/usage?unit='+unit+'&count='+cnt)).data||[];}catch(e){d=[];}
  drawChart(d);
}
...
window.addEventListener('resize',()=>{if(document.getElementById('panel-sum').classList.contains('active'))loadUsage();});
loadTotals();loadUsage();
</script>
</body>
</html>
"""
...
class Engine:
    ...
    async def generate(self, ids, max_tokens, eos, drafter=None, sample=None,
                       ...
                       after=None, escape=(), think=None, seg=None):   # noqa: E301
...
def build_app(tok, engine, ctx, max_cap=4096, queue_timeout=600):
    ...
    async def report_usage(unit: str = "day", count: int = 30):
        unit = unit if unit in ("hour", "day", "week", "month") else "day"
        count = max(1, min(count, 24 * 60))
        return {"success": True, "unit": unit, "count": count,
                "data": engine.ledger.usage(unit, count)}
    ...
    def render_with_images(msgs, kw, imgs):
        """-> (ids, snap, snap2, wire_imgs, seg). ONE renderer for BOTH wires.
        ...
        """
        try:
            ids, snap, snap2, seg = render_prompt(msgs, kw)
        ...
        if not imgs:
            return ids, snap, snap2, None, seg
    ...
    async def serve(ids, max_tokens, stops, stream, chat, prefix,
                    ...
                    http_request=None, endpoint=None):
        ...
        # beside the endpoint name and the client address. `endpoint` is
        # passed by the route rather than derived from `wire`: wire says how
        # the bytes are serialized, the route says who asked.
        ctx = {
            "endpoint": endpoint or ("/v1/" + ("responses" if wire == "responses"
    ...
    async def chat_completions(req: ChatReq, http_request: Request):
        ...
        ids, snap, snap2, wire_imgs, seg = render_with_images(msgs, kw, imgs)
        if pre:
